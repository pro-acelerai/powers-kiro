...
export const submitThinkToolDefinition = {
  name: 'modernization_submit_think',
  description:
    'Record an intermediate reasoning step during any modernization phase. ' +
    'Does not change session state — just persists the reasoning for auditability. ' +
    'Use before major submissions (findings, spec, plan, files) to document your reasoning chain.',
  inputSchema: {
    ...
    properties: {
      ...
      phase: {
        type: 'string',
        description: 'Label for this think step (e.g. "legacy analysis", "spec extraction", "architecture decision")',
      },
      reasoning: {
        type: 'string',
