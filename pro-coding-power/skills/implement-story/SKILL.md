...
Call `pro-coding_create_session` with:
...
Call `pro-coding_read_scope` with `sessionId`.
...
Reason about the requirements against the files you read. Then call `pro-coding_submit_plan` with:
- `sessionId`
...
For **each file** you need to create, modify, or delete, call `pro-coding_submit_proposed_change` with:
- `sessionId`
...
Once they respond, call `pro-coding_request_human_approval` with:
- `sessionId`
...
Call `pro-coding_apply_changes` with `sessionId`.
...
Call `pro-coding_run_lint` with `sessionId`.
...
For each file that needs fixing, call `pro-coding_submit_correction` with:
- `sessionId`
